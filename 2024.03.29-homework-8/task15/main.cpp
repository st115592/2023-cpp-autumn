...
bool CGraph::TourGraph(int v, int e)
{
    if (e != v * (v - 1) / 2)
    {
        return false;
    ...
    for (int i = 0; i < v; ++i)
    {
        for (int j = 0; j < v; ++j)
        {
            if (i != j)
            {
                bool connectedAB = false;
                for (int k = 0; k < e; ++k)
                {
                    if (_edge[k].a == i + 1 && _edge[k].b == j + 1)
                    {
                        connectedAB = true;
                        break;
                    }
                ...
                if (!connectedAB)
                {
                    bool connectedBA = false;
                    for (int k = 0; k < e; ++k)
                    {
                        if (_edge[k].a == j + 1 && _edge[k].b == i + 1)
                        {
                            connectedBA = true;
                            break;
                        }
