...
public:
	...
	int WrongRoadsCounter();
    void VertexColor(int color, int v);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i <= v; ++i)
    {
        int color = 0;
        std::cin >> color;
        g.VertexColor(color, i);
    }

    std::cout << g.WrongRoadsCounter();
...
void CGraph::VertexColor(int color, int v)
{
    for (int i = 0; i < edgesCount(); ++i)
    {
        if (_edge[i].a == v)
